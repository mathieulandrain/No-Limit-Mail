 ...
 * @throws {Error}
 */
async function createNewThreadForUser(
  user,
  quiet = false,
  ...
  if (existingThread) {
    throw new Error(
      "Vous avez tenté de créer un nouveau thread pour un utilisateur avec un thread ouvert existant!"
    );
  }
  ...
  if (!quiet) {
    ...
    if (config.mentionRole) {
      await newThread.postNonLogMessage({
        content: `${utils.getInboxMention()}Nouveau NoLimitMail de (${
          newThread.user_name
        })`,
  ...
  infoHeaderItems.push(`AGE DU COMPTE **${accountAge}**`);
  ...
  for (const [guildId, guildData] of userGuildData.entries()) {
    ...
    const headerItems = [
      `NOM **${utils.escapeMarkdown(nickname)}**`,
      `NOUS A REJOINS IL A **${joinDate}**`,
    ];
  ...
  if (userLogCount > 0) {
    infoHeader += `\n\nCette utilisateur **${userLogCount}** a déjà fait des NoLimit Mail. Utilise \`${config.prefix}logs\` pour les voir.`;
  }
  ...
  if (responseMessageError) {
    await newThread.postSystemMessage(
      `**NOTE:** Could not send auto-response to the user. The error given was: \`${responseMessageError.message}\``
    );
  }
