...
class Thread {
  ...
  async replyToUser(
    ...
    if (this.scheduled_close_at) {
      ...
      await this.postSystemMessage(
        `Annulation de la fermeture programmée de ce fil en raison d'une nouvelle réponse`
      );
    }
  ...
  async receiveUserReply(msg) {
    ...
    if (this.scheduled_close_at) {
      await this.cancelScheduledClose();
      await this.postSystemMessage(
        `<@!${this.scheduled_close_id}> Le fil qui devait être fermé a reçu une nouvelle réponse. Annulation.`
      );
    }
    ...
    if (this.alert_id) {
      ...
      await this.postSystemMessage(
        `<@!${this.alert_id}> Nouveau message de ${this.user_name}`
      );
    }
  }
  ...
  async postToUser(text, file = null) {
    ...
    if (!dmChannel) {
      throw new Error(
        "Impossible d'ouvrir les DM avec l'utilisateur. Il a peut-être bloqué le bot ou défini leurs paramètres de confidentialité plus haut."
      );
    }
  ...
  async close(suppressSystemMessage = false, silent = false) {
    if (!suppressSystemMessage) {
      console.log(`Fermeture ${this.id}`);

      if (silent) {
        await this.postSystemMessage("Fermeture en silence...");
      } else {
        await this.postSystemMessage("Fermeture...");
      }
    }
    ...
    if (channel) {
      console.log(`Channel supprimé ${this.channel_id}`);
      await channel.delete("Aide fermé.");
    }
  }
