...
if (foundConfigFile) {
  ...
  try {
    if (foundConfigFile.endsWith(".js")) {
      ...
      if (
        foundConfigFile.endsWith(".ini") ||
        foundConfigFile.endsWith(".ini.txt")
      ) {
...
const defaultConfig = {
  ...
  status: "Envoyez moi un message",
  responseMessage: "Merci pour ton message, on te répond au plus vite.",
  closeMessage: null,
  ...
  accountAgeDeniedMessage:
    "Votre compte Discord n'est pas assez ancien pour contacter No Limit Mail.",

  requiredTimeOnServer: null, // In minutes
  timeOnServerDeniedMessage:
    "Vous n'êtes pas membre du serveur depuis assez longtemps pour contacter No Limit Mail.",
...
for (const [key, value] of Object.entries(process.env)) {
  ...
  userConfig[configKey] = value.includes("||") ? value.split("||") : value;
...
if (loadedEnvValues > 0) {
  console.log(
    `Loaded ${loadedEnvValues} ${
      loadedEnvValues === 1 ? "value" : "values"
...
// Specific checks
if (
  finalConfig.attachmentStorage === "discord" &&
  !finalConfig.attachmentStorageChannelId
) {
  console.error(
    "Config option 'attachmentStorageChannelId' is required with attachment storage 'discord'"
  );
  process.exit(1);
}
...
for (const numericOpt of numericOptions) {
  if (finalConfig[numericOpt] != null) {
    ...
    if (Number.isNaN(number)) {
      console.error(
        `Invalid numeric value for ${numericOpt}: ${finalConfig[numericOpt]}`
      );
      process.exit(1);
    }
